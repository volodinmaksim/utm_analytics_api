...
app = FastAPI(title="UTM Analytics", version="0.5.0")
...
<div class="max-w-6xl mx-auto p-6">
  ...
  <div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
    <div>
      ...
      <p class="text-slate-600">BASE_URL: <code>__BASE_URL__</code></p>
    </div>
    ...
    <div class="flex gap-2">
      ...
      <button id="reload" class="rounded-lg px-4 py-2 bg-slate-900 text-white">
        Обновить
      </button>
    </div>
  </div>
  ...
  <div class="mt-8">
    <h2 class="text-xl font-bold mb-3">UTM-метки (откуда пришли)</h2>
    <div class="overflow-x-auto bg-white rounded-2xl shadow">
      <table class="min-w-full text-sm">
        <thead class="bg-slate-100">
          <tr>
            <th class="text-left p-3">utm_mark</th>
            <th class="text-left p-3">users</th>
          </tr>
        </thead>
        <tbody id="utm_marks" class="divide-y"></tbody>
      </table>
    </div>
  </div>

  <div id="branches_wrap" class="mt-8 hidden">
    <h2 class="text-xl font-bold mb-3">Ветки пользователей (RPP)</h2>
    <div class="overflow-x-auto bg-white rounded-2xl shadow">
      <table class="min-w-full text-sm">
        <thead class="bg-slate-100">
          <tr>
        ...
        <tbody id="branches" class="divide-y"></tbody>
      </table>
    </div>
  </div>

  <div id="segments_wrap" class="mt-8 hidden">
  ...
  <div class="mt-8">
    <h2 class="text-xl font-bold mb-3">Пожелания пользователей</h2>
...
async function load() {
  ...
  el("utm_marks").innerHTML = "";
  el("branches").innerHTML = "";

  el("segments_wrap").classList.add("hidden");
  el("branches_wrap").classList.add("hidden");
  ...
  if (Array.isArray(data.utm_marks)) {
    for (const row of data.utm_marks) {
      const tr = document.createElement("tr");
      tr.innerHTML = `
        <td class="p-3">${row.utm_mark ?? ""}</td>
        <td class="p-3">${fmt(row.users)}</td>
      `;
      el("utm_marks").appendChild(tr);
    }
  }

  if (service === "rpp" && Array.isArray(data.branches)) {
    el("branches_wrap").classList.remove("hidden");
...
async def analytics(
    ...
    try:
        ...
        utm_marks = await exec_all(session, queries.utm_marks(service))

        segments = None
        branches = None
        if service == "rpp":
            segments = await exec_all(session, queries.segments_rpp())
            branches = await exec_all(session, queries.branches_rpp())
